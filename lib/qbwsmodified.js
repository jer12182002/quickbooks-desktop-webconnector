...
//return true if the item is found or created, or return false if there is something in this process
}


 
var chkOrCreateCustomer = (customerData) => {
    console.log(chalk.red(customerData.first_name));

//return true if the customer is found or created, or return false if there is something in this process
}
...
var buildInvoiceRequest = (shopifyData) => {
    console.log(chalk.green("Build Invoice Request"));
    var strRequestXML,inputXMLDoc,data;

    data = JSON.parse(shopifyData);
    chkOrCreateCustomer(data.customer);
    
    // if(chkOrCreateCustomer()&&chkOrCreateItem()){
        //buildInvoiceQuery
    // }
    // else{
    //    console.log(chalk.red('########### There is soemthing wrong ###############'));
    // }
...
//    Invoice Add Query
    ...
    //             'InvoiceAddRq' : {
    //                     '@requestID' : data.number,
    //                     'InvoiceAdd' : {
    //                         '@defMacro' : 'MACROTYPE',
    //                         'CustomerRef' : {
    //                             'ListID' : data.customer.id,
    //                             'FullName' : data.customer.first_name + ' ' + data.customer.last_name                       
    //                         },
    //                         'TxnDate' : data.created_at.substr(0,10),
    //                         'RefNumber' : data.number,
    //                         'BillAddress' : {
    //                             'Add1' : data.customer.default_address.address1,
    //                             'Add2' : data.customer.default_address.address2,
    ...
    //                         },
    //                         'ShipAddress' : {
    //                             'Add1' : data.shipping_address.address1,
    ...
    // strRequestXML = builder.create(inputXMLDoc, { encoding: 'utf-8' })
    //                         .instruction('qbxml', 'version="4.0"');

    // data.line_items.forEach((item) => {
    //     var lineItem = strRequestXML.ele('InvoiceLineAdd');
    //     lineItem.ele('ItemRef')
    //         .ele('FullName',item.title);
    //     lineItem.ele('Quantity',item.quantity);
    //     lineItem.ele('Rate',item.price);    
    // });

    // console.log(chalk.red(strRequestXML.end( { pretty : true } ) ) );
    // testXML = strRequestXML;
    // console.log(chalk.green(testXML));

    //var testXML = '<?xml version="1.0" encoding="utf-8"?><?qbxml version="10.0"?><QBXML><QBXMLMsgsRq onError="stopOnError"><InvoiceAddRq requestID="2"><InvoiceAdd><CustomerRef><FullName>Keith Palmer</FullName></CustomerRef><TxnDate>2007-12-14</TxnDate><RefNumber>9668</RefNumber><BillAddress><Addr1>56 Cowles Road</Addr1><City>Willington</City><State>CT</State><PostalCode>06279</PostalCode><Country>United States</Country></BillAddress><PONumber></PONumber><ItemSalesTaxRef><FullName>CT Tax</FullName></ItemSalesTaxRef><Memo></Memo><InvoiceLineAdd><ItemRef><FullName>Test Item</FullName></ItemRef><Desc>Item 1 Description Goes Here</Desc><Quantity>1</Quantity><Rate>295</Rate></InvoiceLineAdd><InvoiceLineAdd><ItemRef><FullName>Test Item</FullName></ItemRef><Desc>Item 2 Description Goes Here</Desc><Quantity>3</Quantity><Rate>25</Rate></InvoiceLineAdd></InvoiceAdd></InvoiceAddRq></QBXMLMsgsRq></QBXML>';
...
function buildRequest() {
   ...
   // request.push(strRequestXML);
    testXML = '<?xml version="1.0" encoding="utf-8"?><?qbxml version="4.0"?><QBXML><QBXMLMsgsRq onError="stopOnError"><CustomerQueryRq><FullName>KO ADWIN</FullName></CustomerQueryRq></QBXMLMsgsRq></QBXML>';
    
   request.push(testXML);
   console.log(chalk.red('build customized query'));
    return request;
}
...
module.exports.run = function runQBWS() {
    ...
    //     console.log(chalk.red(config.verbosity));
    //     soapServer.log = function soapServerLog(type, data) {
    //       if(type == 'received') {
    //         var shopifyData = JSON.parse(data);
    //         console.log(chalk.red(shopifyData));
    //     }
